...
import org.lwes.oem.ObjectEventMapper;
...
public class ArrayEventPerformanceTest {
    ...
    public void direct() throws Exception {
        ...
        // Compute "stats" as the number of ArrayEvent operations performed
        // during the test.
        ...
        final String message = String.format("Scaled CPU time was %f, and was expected to be around %f", scaledTime, DIRECT_GET_CPU_TIME);
        LOG.info(message);
    ...
    public void oem() throws Exception {
        ...
        final EventStruct struct = new EventStruct();
        final ObjectEventMapper<EventStruct> oem = ObjectEventMapper.construct(struct);
        for (int p = 0; p<NUM_PASSES; ++p) {
            for (ArrayEvent event : events) {
                oem.load(event);
            }
        ...
        assertEquals(0, stats.get(ArrayEventStats.COPIES).intValue());
        assertEquals(0, stats.get(ArrayEventStats.SWAPS).intValue());
        assertEquals(numFields*NUM_PASSES, stats.get(ArrayEventStats.PARSES).intValue());
        ...
        LOG.info(message);
        assertTrue(message, scaledTime<=OEM_GET_CPU_TIME*TOLERANCE);
    }
    ...
    private static final class EventStruct {
        public int    the_trax_time;
        public String the_trax_id;
        public long   the_event_id;
